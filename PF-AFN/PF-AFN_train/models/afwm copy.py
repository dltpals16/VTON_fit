...
class AFWM(nn.Module):
    def __init__(self, opt, input_nc):
        """
        input_nc: input channel count for condition input (e.g., parse_agnostic + densepose + fitmap)
        """
        super(AFWM, self).__init__()
        num_filters = [64,128,256,256,256]

        # image_input: warped cloth (3ch)
        self.image_features = FeatureEncoder(3, num_filters)

        # cond_input: parse_agnostic + densepose + fitmap (e.g., 18 + 3 + 3)
    ...
    def forward(self, cond_input, image_input, image_edge):
        """
        cond_input: (B, input_nc, H, W)
        image_input: warped cloth image (B, 3, H, W)
        image_edge: warped cloth mask or edge map (B, 1, H, W)
        """
    ...
    def update_learning_rate_warp(self, optimizer):
        ...
        for param_group in optimizer.param_groups:
            param_group['lr'] = lr
        if opt.verbose:
            print('update learning rate (warp): %f -> %f' % (self.old_lr_warp, lr))
        self.old_lr_warp = lr
